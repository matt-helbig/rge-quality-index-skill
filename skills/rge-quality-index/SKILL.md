...
QI evaluates the email artifact and the strategic context actually supplied. It does not certify legal compliance, deliverability, ESP configuration, or revenue impact. An element being present does not establish that it functions or meets every applicable requirement. Keep jurisdiction-specific and untested client requirements separate from generic scoring. Where a judgment depends on what is currently common rather than on a durable rule, say so in the review and verify it against an authoritative source. This skill is not maintained as a snapshot of current practice, so do not treat any part of it as one.
