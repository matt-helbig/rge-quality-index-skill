...
- `cfo_metric_impact` is a boolean, not a text hypothesis or the string `"false"`. When true, include a testable `cfo_metric_hypothesis`.
- `cfo_criteria_met` is an integer from 0 to 6. Document each met criterion in `cfo_criteria_evidence` using `criterion` and `evidence`; do not count synonyms twice. Use the six names in [scoring.md](scoring.md). The calculator validates arithmetic and scalar inputs; the reviewer must validate the underlying claims and evidence.
